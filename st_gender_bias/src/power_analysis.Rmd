...
power_analysis <- function(d, quote_col_name='quote_mean', n=NULL) {
    power       <- 0.8
    sig_level   <- 0.05
    ...
    sd_null     <- d[treatment==0, sd(get(quote_col_name))] 
    sd_alt      <- d[treatment==1, sd(get(quote_col_name))] 
    if (is.null(n)) {
        cohen.d     <- (m_alt - m_null) / sqrt((sd_null^2 + sd_alt^2) / 2) 
    } else {
        cohen.d <- NULL
    }
    ...
    # recover effect size
    if (is.null(cohen.d)) {
        es <- pwr$d * sqrt((sd_null^2 + sd_alt^2) / 2)
        return(list(pwr, es))
    } else {
        return(pwr)
    }
