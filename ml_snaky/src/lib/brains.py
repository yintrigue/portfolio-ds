...
def enable_brain_v1(
    ...
    if build_training_data:
        snake.simulate(n_game=30000, csv='./data/train.csv', vision_mode='simple')
    ...
    if do_training:
        snake.train(
                ...
                epoch=10,
                batch_size=32,
                weights_save_path='./models/brain_v1_lr.h5')

        # testing model
        snake.evaluate(
                brain_model='logi',
                vision='simple',
                model_weights='./models/brain_v1_lr.h5',
...
def enable_brain_v2(
    ...
    if do_training:
        snake.train(
                ...
                epoch=10,
                batch_size=32,
                weights_save_path='./models/brain_v2_nn.h5')

        # testing model
        snake.evaluate(
                brain_model='neural_net',
                vision='simple',
                model_weights='./models/brain_v2_nn.h5',
                test_csv='./data/train.csv',
                sig_thd=0.5)
...
def enable_brain_v3(
    ...
    # generate data
    if build_training_data:
        snake.simulate(n_game=30000, csv='./data/train.csv', vision_mode='advanced')
    ...
    if do_training:
        snake.train(
                ...
                epoch=20,
                batch_size=32,
                weights_save_path='./models/brain_v3_nn.h5')

        # testing model
        snake.evaluate(
                brain_model='neural_net',
                vision='advanced',
                model_weights='./models/brain_v3_nn.h5',
                test_csv='./data/train.csv',
                sig_thd=0.5)
...
def enable_brain_v4(
    ...
    if do_training:
        snake.train(
                ...
                epoch=30,
                batch_size=32,
                weights_save_path='./models/brain_v4_nn.h5')

        # testing model
        snake.evaluate(
                brain_model='neural_net',
                vision='expert',
                model_weights='./models/brain_v4_nn.h5',
...
def build_train_data_using_brain_v3(
    ...
    if do_training:
        ...
        snake.simulate(n_game=N_GAME_RANDOM, csv='./data/train.csv', vision_mode='advanced')
        snake.train(
                brain_model='neural_net',
                vision='advanced',
                csv='./data/train.csv',
    ...
    df1 = pd.read_csv('./data/train.csv', dtype={
                                            'vision': np.object,
                                            'vision_simple': np.object,
                                            'vision_advanced': np.object,
                                            'vision_expert': np.object,
                                            'vision_new': np.object})
    df2 = pd.read_csv('./data/train_m3.csv', dtype={
                                            'vision': np.object,
                                            'vision_simple': np.object,
                                            'vision_advanced': np.object,
                                            'vision_expert': np.object,
                                            'vision_new': np.object})
    df1.loc[:,'game_id'] += 3000000
    df2.loc[:,'game_id'] += 2000000
    df = pd.concat([df1, df2], ignore_index=True)
    df.to_csv('./data/train.csv', index=False)
    os.remove("./data/train_m3.csv")
    print("Traning data have been built!")
